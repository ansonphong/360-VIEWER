...
def scan_directory(root_dir):
    ...
    missing_thumbnails = []
    missing_jpgs = []
    total_files = sum([len(files) for r, d, files in os.walk(root_dir) if '_thumbnails' not in r and '_Q' not in r])
    
    with tqdm(total=total_files, desc="Scanning files") as pbar:
        for dirpath, dirnames, filenames in os.walk(root_dir):
            # Skip the _thumbnails and _Q* folders
            if '_thumbnails' in dirpath or '_Q' in dirpath:
                continue
...
def generate_thumbnail_path(root_dir, file_path):
    ...
    full_path = os.path.join(root_dir, '_thumbnails', thumbnail_name).replace(os.sep, '/')
    return full_path, f"_thumbnails/{thumbnail_name}"
...
def generate_jpg_path(root_dir, file_path, quality):
    ...
    full_path = os.path.join(root_dir, f'_{quality}', jpg_name).replace(os.sep, '/')
    return full_path, f"_{quality}/{jpg_name}"
        ...
        for original_path, q100_path, q75_path, q50_path in missing_jpgs:
            try:
                with Image.open(original_path) as img:
                    ...
                    img.save(q100_path, "JPEG", quality=100)
                    pbar.update(1)
                    img.save(q75_path, "JPEG", quality=75)
                    pbar.update(1)
                    img.save(q50_path, "JPEG", quality=50)
                    pbar.update(1)
            except Exception as e:
