	...
	class Phong360ViewerCore {
		constructor(options = {}) {
			...
			if (typeof THREE === 'undefined') {
				throw new Error(
					'Phong360ViewerCore requires Three.js. Please load Three.js before initializing the viewer.'
				);
			}
			...
			try {
				...
				if (savedProjection !== null) {
					...
					console.log(
						'[Phong360ViewerCore] Loaded saved projection preference:',
		...
		setupEventListeners() {
			...
			document.addEventListener('wheel', this.boundHandlers.onDocumentMouseWheel, {
				passive: false
			});

			// Touch events (non-passive to allow preventDefault)
			this.container.addEventListener('touchstart', this.boundHandlers.onTouchStart, {
				passive: false
			});
			this.container.addEventListener('touchmove', this.boundHandlers.onTouchMove, {
				passive: false
			});
			this.container.addEventListener('touchend', this.boundHandlers.onTouchEnd, {
				passive: false
			});
		...
		switchProjection(type) {
			...
			try {
				...
				console.log(
					'[Phong360ViewerCore] Saved projection preference:',
					type === 0 ? 'gnomonic' : 'stereographic'
				);
			} catch (e) {
		...
		onPointerMove(event) {
			...
			const sensitivity =
				(this.state.fov / this.container.clientWidth) * this.config.sensitivity.mouse;
			this.targetState.lon =
				(this.pointerStartX - event.clientX) * sensitivity + this.onPointerDownLon;
			this.targetState.lat =
				(event.clientY - this.pointerStartY) * sensitivity + this.onPointerDownLat;
		...
		startContinuousPan() {
			...
			this.panInterval = setInterval(() => {
				...
				if (this.activeKeys.panUp)
					this.targetState.lat = Math.min(85, this.targetState.lat + panSpeed);
				if (this.activeKeys.panDown)
					this.targetState.lat = Math.max(-85, this.targetState.lat - panSpeed);
			}, 16);
		}

		stopContinuousPan() {
			if (
				!this.activeKeys.panLeft &&
				!this.activeKeys.panRight &&
				!this.activeKeys.panUp &&
				!this.activeKeys.panDown
			) {
		...
		onTouchMove(event) {
			...
			if (event.touches.length === 1 && this.isTouchDragging) {
				...
				// Degrees-per-pixel from FOV and container size, scaled by config multiplier
				const sensitivity =
					(this.state.fov / this.container.clientWidth) * this.config.sensitivity.touch;
		...
		update() {
			...
			if (this.config.viewRotation.autoRotate && !this.isUserInteracting) {
				this.targetState.lon =
					this.targetState.lon -
					this.config.viewRotation.autoRotationRate * this.state.azimuthSign * safeDelta;
			}
			...
			if (!this.isTouchDragging && !this.isPinching) {
				...
				if (
					isNaN(this.state.theta) ||
					isNaN(this.state.phi) ||
					isNaN(this.state.fov) ||
					isNaN(this.state.lon) ||
