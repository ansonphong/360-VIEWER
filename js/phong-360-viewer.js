	...
	class Phong360Viewer {
		constructor(options = {}) {
			...
			if (typeof THREE === 'undefined') {
				throw new Error(
					'Phong360Viewer requires Three.js. Please load Three.js before initializing the viewer.'
				);
			}
		...
		onKeyUp(event) {
			if (this.activeKeys.hasOwnProperty(event.key)) {
				...
				if (event.key === '=' || event.key === '+' || event.key === '-' || event.key === '_') {
					...
					if (
						!this.activeKeys.ArrowUp &&
						!this.activeKeys.ArrowDown &&
						!this.activeKeys.ArrowLeft &&
		...
		updateProjectionButton(type) {
			if (this.uiElements.switchProjectionButton) {
				this.uiElements.switchProjectionButton.title =
					type === 0 ? 'Gnomonic Projection' : 'Stereographic Projection';
				this.uiElements.switchProjectionButton.textContent =
