...
class Phong360Engine {
  ...
  destroy(): void;                                      // disposes Three.js renderer, removes canvas, removes all DOM children of container, removes all event listeners, cancels in-flight fetches (see scope note below), nulls internal refs. Safe to call at any lifecycle stage (before loadLibrary, mid-load, after errors). Idempotent.
                                                        //   Cancellation scope: manifest fetches (via AbortController) ARE cancelled. In-flight Three.js TextureLoader requests are NOT directly abortable (Three.js TextureLoader has no abort API) — they complete to the network and are then dropped via the load-token freshness check. No engine events fire from late texture completions after destroy().
